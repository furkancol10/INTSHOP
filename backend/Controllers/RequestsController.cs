...
public class RequestsController : ControllerBase
{
    ...
    public async Task<IActionResult> Approve(int id, [FromBody] KararReq? req)
    {
        ...
        if (user.Value.Role != "Admin") return StatusCode(403, "Sadece Admin");
        ...
        await Denetim.Yaz(_db, HttpContext, user.Value.Id, user.Value.Role,
            "request.approve", "requests", id,
            oldValue: new { status = "pending" },
            newValue: new { status = "approved", uygulanan_fiyat = yeni_Fiyat, dealerId, productId, note = req?.note });
    ...
    public async Task<IActionResult> Reject(int id, [FromBody] KararReq? req)
    {
        ...
        await Denetim.Yaz(_db, HttpContext, user.Value.Id, user.Value.Role,
            ...
            oldValue: new { status = "pending" },
            newValue: new { status = "rejected", note = req?.note });
