...
- **Manuel token doğrulaması:** 8 controller'dan 6'sı `GetUser`/`GetRole` kodunu kopyalamış. `[Authorize]` yok, authentication middleware yok. Token'ın süresi/iptali yok; token DB'de düz metin saklanıyor (login'de SELECT, register'da hash'lenmeden).
...
**Öneri:** Tek transaction içinde `SELECT ... FOR UPDATE` (veya `UPDATE dealer_stock SET stock = stock + @change WHERE ... AND stock + @change >= 0` gibi koşullu güncelleme) + hareket kaydı. `Program.cs`'e IF (Oracle) değil; Dapper'da `IDbTransaction` ile.
